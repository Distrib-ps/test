...
exports.commands = {
    ...
    forcejoin: function(target, room, user) {
  	...
  	listroom: function (target, room, user) {
	...
	customgdeclare: function (target, room, user) {
		...
		for (let id in Rooms.rooms) {
			if (id !== 'global') Rooms.rooms[id].addRaw('<div class="broadcast-blue" style="border-radius: 5px;"><b>Nous organisons un <font color="#57194A"><b>' + parts[0] + '</b></font> join <button name="send" value="/join ' + parts[1] + '" style="border-radius: 3px; margin: 3px; padding: 2px 5px; font-weight: bold; font-style: italic; box-shadow: 0px 0px 2px rgba(0, 0, 0, 0.35); color: #57194A; text-shadow: none;">' + parts[1] + '</button> !<br />Le tier est<font style="color: #57194A; font-weight: bold;"><b>' + parts[2] + '</b></font>! Rejoignez-nous et amusez-vous!<br /><br />Le prix a gagner est <font style="color: #57194A; font-weight: bold;"><b>' + parts[3] + '</b></font> Le second gagnera <font style="color: #57194A; font-weight: bold;"><b>' + parts[4] + '</b></font> bucks!<br /><small><i>~' + user.name + '</i></small></b></div>');
		}
		this.logModCommand(user.name + " globally custom declared " + target);
	},
	customgdeclarehelp: ["/customgdeclare [event name], [room], [tier], [buck reward], [runner-up buck reward] - Gdeclare prédéfini qui annonce anonymement un message dans chaque pièce du serveur. Nécessite: &, ~"],
