...
exports.commands = {
 ...
 dm: 'daymute',
	daymute: function (target, room, user) {
		...
		this.addModCommand('' + targetUser.name + ' A été mute par ' + user.name + ' pendant 24 heures.' + (target ? " (" + target + ")" : ""));
		var alts = targetUser.getAlts();
		if (alts.length) this.addModCommand("" + targetUser.name + "Ces alts on était mute! " + alts.join(", "));
